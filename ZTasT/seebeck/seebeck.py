...
# === 平滑曲线函数 ===
def smooth_curve(x, y, n_points=500):
    if len(x) < 4:
        return x, y
...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            Seebeck_original = subset['seebeck'].values
            label_name = f"{strain_labels[strain]} - {direction_labels[direction]}"
            
            # 生成平滑曲线
            T_smooth, Seebeck_smooth = smooth_curve(T_original, Seebeck_original)
            ...
            ax.plot(T_smooth, Seebeck_smooth,
                    color=colors[strain],
                    linestyle='-',
                    linewidth=2.5,
...
ax.set_ylabel(r'Seebeck coefficient, $\mathbf{S}$ ($\mathbf{\mu}$V/K)', fontweight='bold')

# 坐标轴范围
ax.set_xlim(df['Temperature'].min() - 50, df['Temperature'].max() + 50)
ax.set_ylim(130, 325)  # 保持您设置的Y轴范围
...
plt.savefig("Seebeck_Coefficient_vs_Temperature_Smooth.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Seebeck_Coefficient_vs_Temperature_Smooth.png", dpi=600, bbox_inches='tight')
plt.show()
