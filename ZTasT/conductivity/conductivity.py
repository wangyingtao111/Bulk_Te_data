...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            E_original = subset['electrivity'].values  # 电导率数据
            label_name = f"{strain_labels[strain]} - {direction_labels[direction]}"
            
            # 生成平滑曲线
            T_smooth, E_smooth = smooth_curve(T_original, E_original)
...
ax.set_ylabel(r'Electrical conductivity, $\mathbf{\sigma}$ (S/m)', fontweight='bold')
...
plt.savefig("Electrical_Conductivity_vs_Temperature_Smooth.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Electrical_Conductivity_vs_Temperature_Smooth.png", dpi=600, bbox_inches='tight')
plt.show()
