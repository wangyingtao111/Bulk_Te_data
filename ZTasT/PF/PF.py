...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            PF_original = subset['PF'].values
            label_name = f"{strain_labels[strain]} - {direction_labels[direction]}"
            
            # 生成平滑曲线
            T_smooth, PF_smooth = smooth_curve(T_original, PF_original)
...
ax.set_ylabel(r'Power factor, $\mathbf{PF}$ (10$\mathbf{^{-3}}$ W m$\mathbf{^{-1}}$ K$\mathbf{^{-2}}$)', fontweight='bold')

# 坐标轴范围
ax.set_xlim(df['Temperature'].min() - 50, df['Temperature'].max() + 50)
# 功率因数让matplotlib自动调整Y轴范围
...
plt.savefig("Power_Factor_vs_Temperature_Smooth.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Power_Factor_vs_Temperature_Smooth.png", dpi=600, bbox_inches='tight')
plt.show()
