...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            # 【核心修改点】：读取 'lattice thermal conductivity' 列进行绘制
            ax.plot(subset['Temperature'], subset['lattice thermal conductivity'], 
                    color=colors[strain], 
...
# 【核心修改点】：更新 Y 轴标签为晶格热导率，使用 \kappa_l 符号
ax.set_ylabel('Lattice thermal conductivity, $\mathbf{\kappa_l}$ (W m$\mathbf{^{-1}}$ K$\mathbf{^{-1}}$)', fontweight='bold')
...
plt.tight_layout() 
# 更新输出文件名以匹配晶格热导率
plt.savefig("Kappa_l_vs_Temperature_NatureStyle_EN.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Kappa_l_vs_Temperature_NatureStyle_EN.png", dpi=600, bbox_inches='tight')
