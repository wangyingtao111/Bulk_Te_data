...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            # 电子热导率数据
            Kappa_original = subset['electrical thermal conductivity'].values
            label_name = f"{strain_labels[strain]} - {direction_labels[direction]}"
            
            # 生成平滑曲线
            T_smooth, Kappa_smooth = smooth_curve(T_original, Kappa_original)
...
ax.set_ylabel(r'Electronic thermal cond., $\mathbf{\kappa_e}$ (W m$^{-1}$ K$^{-1}$)', fontweight='bold')

# 坐标轴范围
ax.set_xlim(df['Temperature'].min() - 50, df['Temperature'].max() + 50)
# 电子热导率让matplotlib自动调整Y轴范围
...
plt.savefig("Kappa_e_vs_Temperature_Smooth.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Kappa_e_vs_Temperature_Smooth.png", dpi=600, bbox_inches='tight')
plt.show()
