...
for strain in strains:
    for direction in directions:
        ...
        if not subset.empty:
            ...
            # 【核心修改点】：读取 'electrical thermal conductivity' 列进行绘制
            ax.plot(subset['Temperature'], subset['electrical thermal conductivity'], 
                    color=colors[strain], 
...
# 【核心修改点】：更新 Y 轴标签为电子热导率，使用 \kappa_e 符号和学术规范单位
ax.set_ylabel('Electronic thermal cond., $\mathbf{\kappa_e}$ (W m$\mathbf{^{-1}}$ K$\mathbf{^{-1}}$)', fontweight='bold')
...
plt.tight_layout() 
# 更新输出文件名以匹配当前的物理量
plt.savefig("Kappa_e_vs_Temperature_NatureStyle_EN.pdf", dpi=600, bbox_inches='tight')
plt.savefig("Kappa_e_vs_Temperature_NatureStyle_EN.png", dpi=600, bbox_inches='tight')
